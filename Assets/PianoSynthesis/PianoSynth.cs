...
{
    // Lives in Boot scene, persists forever.
    // Assign noteClips in the Inspector.
    // Name convention: index 0 = C3, index 1 = C#3, index 2 = D3, ...
    // covering C3 (MIDI 48) through B5 (MIDI 83) = 36 clips.
    public class PianoSynth : MonoBehaviour
    {
        public static PianoSynth Instance { get; private set; }

        [Header("Samples")]
        [Tooltip("Chromatic piano clips starting from baseMidiPitch.")]
        ...
        AudioSource[]            _pool;
        int                      _poolHead;
        Dictionary<int, int>     _clipIndex = new(); // midiPitch → clip array index
